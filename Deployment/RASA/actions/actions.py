...
#
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        cursor = con.execute(
            "SELECT company_official from rasa_chatbot_bursalist WHERE company_tickname='{}'".format(slot_value))
        row = cursor.fetchone()
        msg = "Today: {}\n\n{} :\n\n**Price-MYR**\n".format(str(d1), row[0])
        ...
        cursor = con.execute(
            "SELECT * from rasa_chatbot_option1_selection WHERE username='{}'".format(username))
        row = cursor.fetchone()
        con.close()
        ...
        if row[19]:
            ...
            if ind:
                msg = msg + "RSI-28: {:.2f}\n".format(ind.rsi()[-1])
        if row[20]:
            ind = PercentagePriceOscillator(
                close=df["Close"],  window_slow=26, window_fast=12, window_sign=9, fillna=True)
            if ind:
                msg = msg + \
                    "Percentage Price Oscillator: {:.2f}\n".format(
                        ind.ppo()[-1])
        if row[21]:
            ind = PercentageVolumeOscillator(
                volume=df["Volume"],  window_slow=26, window_fast=12, window_sign=9, fillna=True)
            if ind:
                msg = msg + \
                    "Percentage Volume Oscillator: {:.2f}\n".format(
                        ind.pvo()[-1])
        if row[22]:
        ...
        if row[23]:
            ind = StochasticOscillator(
                close=df["Close"],  high=df["High"], low=df["Low"], window=14, smooth_window=3, fillna=True)
            if ind:
                msg = msg + \
                    "Stochastic Oscvillator: {:.2f}\n".format(ind.stoch()[-1])
        if row[25]:
            ind = AccDistIndexIndicator(
                close=df["Close"],  high=df["High"], low=df["Low"], volume=df["Volume"], fillna=True)
            if ind:
        ...
        if row[26]:
            ...
            if ind:
                msg = msg + \
                    "Chaikin Money FLow: {:.2f}\n".format(
                        ind.chaikin_money_flow()[-1])
        if row[27]:
            ind = NegativeVolumeIndexIndicator(
                close=df["Close"], volume=df["Volume"], fillna=True)
            if ind:
                msg = msg + \
                    "Negative Volume Index: {:.2f}\n".format(
                        ind.negative_volume_index()[-1])
        if row[28]:
            ...
            if ind:
                msg = msg + \
                    "OnBalance Volume: {:.2f}\n".format(
                        ind.on_balance_volume()[-1])
        if row[29]:
            ind = VolumePriceTrendIndicator(
                close=df["Close"], volume=df["Volume"], fillna=True)
            if ind:
                msg = msg + \
                    "Volume Price Trend: {:.2f}\n".format(
                        ind.volume_price_trend()[-1])
        if row[30]:
            ind = VolumeWeightedAveragePrice(
                close=df["Close"], high=df["High"], low=df["Low"], volume=df["Volume"], window=14, fillna=True)
            if ind:
                msg = msg + "Volume Weighted Average Price: {:.2f}\n".format(
                    ind.volume_weighted_average_price()[-1])

        # Trading View
        tradingviewlink = "https://www.tradingview.com/chart/?symbol={}".format(
            slot_value)
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if authentication == False:
            ...
            dispatcher.utter_message(
                text="Hi, your session might be expired. Kindly key in your username and password again.", buttons=buttons, button_type=button_type)
            return []
        else:
            # Get today date
            today = date.today()
            ...
            con = sqlite3.connect("../Django/db.sqlite3")
            cursor = con.execute(
                "SELECT top10 from rasa_chatbot_sentimentanalysistop10 WHERE categories='sentime_result_top_10_mentioned'")
            row1 = cursor.fetchone()
            cursor = con.execute(
                "SELECT top10 from rasa_chatbot_sentimentanalysistop10 WHERE categories='Most_Mentioned'")
            row2 = cursor.fetchone()
            cursor = con.execute(
                "SELECT top10 from rasa_chatbot_sentimentanalysistop10 WHERE categories='other'")
            row3 = cursor.fetchone()
            msg = msg + \
                "This sentiment analysis is scheduled to run at midnight 00:00am everyday to update the latest top10 most mentioned stock pick.{}\n\n".format(
                    row3[0])
...
#
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if authentication == False:
            ...
            username = tracker.get_slot("username")
            cursor = con.execute(
                "SELECT company_official from rasa_chatbot_option3_stock_monitoring WHERE username='{}'".format(username))
            row = cursor.fetchall()
            con.close()
...
#
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        con = sqlite3.connect("../Django/db.sqlite3")
        cursor = con.execute(
            "SELECT company_tickname from rasa_chatbot_bursalist WHERE company_official='{}'".format(slot_value.upper()))
        row = cursor.fetchone()
        con.close()
        ...
        if slot_value == "Invalid":
            ...
            x = requests.post(
                "http://127.0.0.1:8000/option3_add/?stock={}&username={}&tick={}".format(slot_value, slot_value1, row[0]))
            buttons = [{"title": "Back to Menu", "payload": "/option3"}]
            button_type = "vertical"
            dispatcher.utter_message(text="{} is added to monitoring list.".format(
                slot_value), buttons=buttons, button_type=button_type)
            return [SlotSet("newstock3", None)]
...
#
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if slot_value == "Invalid":
            ...
            x = requests.post(
                "http://127.0.0.1:8000/option3_delete/?stock={}&username={}&tick={}".format(slot_value, slot_value1, row[0]))
            buttons = [{"title": "Back to Menu", "payload": "/option3"}]
            button_type = "vertical"
            dispatcher.utter_message(text="{} is deleted from monitoring list.".format(
                slot_value), buttons=buttons, button_type=button_type)
            return [SlotSet("deletestock3", None)]
...
#
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        test = data.DataReader([stock1, stock2], 'yahoo',
                               start=startdate, end=d1)
        test = test['Adj Close']
        ...
        row1 = cursor.fetchone()
        cursor = con.execute(
            "SELECT company_official from rasa_chatbot_bursalist WHERE company_tickname='{}'".format(slot_value1))
        row2 = cursor.fetchone()
        con.close()

        buttons = [{"title": "Back to Menu", "payload": "/option4"}]
        button_type = "vertical"
        dispatcher.utter_message(text="Correlation between {} and {} for last {} months is {:.0f}%.".format(
            row1[0], row2[0], month, corr), buttons=buttons, button_type=button_type)
...
#
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        buttons = [{"title": "Back to Menu", "payload": "/option4"}]
        button_type = "vertical"
        dispatcher.utter_message(text="Price Volatility of {} for last {} months is {:.0f}%.".format(
            row1[0], month, volatility), buttons=buttons, button_type=button_type)
...
#
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        for stock in slot_value:
            cursor = con.execute(
                "SELECT company_tickname from rasa_chatbot_bursalist WHERE company='{}'".format(stock.lower()))
            row = cursor.fetchone()
        ...
        for counter, symbol in enumerate(test.columns.tolist()):
            cursor = con.execute(
                "SELECT company_official from rasa_chatbot_bursalist WHERE company_tickname='{}'".format(symbol[:-3]))
            row = cursor.fetchone()
        ...
        button_type = "vertical"
        dispatcher.utter_message(
            text=msg, buttons=buttons, button_type=button_type)
...
#
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        try:
            ...
            # Testing Accuracy
            df = yf.download("{}.KL".format(slot_value),
                             start=startdate, end=d1)
            train_data, test_data = df[0:int(
                len(df)*0.7)], df[int(len(df)*0.7):]
            ...
            for time_point in range(N_test_observations):
                model = ARIMA(history, order=(
                    int(arima_p[0]), int(arima_p[1]), int(arima_p[2])))
            ...
            msg = "Result of ARIMA({},{},{}) - {}\n".format(
                arima_p[0], arima_p[1], arima_p[2], row[0])
            msg = msg + \
                'Testing Mean Squared Error (Most recent 30% of data is testing dataset) : {:.3}\n\n'.format(
                    MSE_error)
            ...
            button_type = "vertical"
            dispatcher.utter_message(
                text="ARIMA is likelty unable to converge.Please re-try.", buttons=buttons, button_type=button_type)
            return [SlotSet("stockoption5", None), SlotSet("arima_parameters", None)]
...
class StockPickForm(FormValidationAction):
    ...
    def validate_stocktick(self,
        ...
        if authentication == False:
            ...
            con = sqlite3.connect("../Django/db.sqlite3")
            cursor = con.execute(
                "SELECT company_tickname from rasa_chatbot_bursalist WHERE company='{}'".format(slot_value.lower()))
            row = cursor.fetchone()
            con.close()
...
class AuthenticationForm(FormValidationAction):
    ...
    def validate_username(self,
        ...
        con = sqlite3.connect("../Django/db.sqlite3")
        cursor = con.execute(
            "SELECT username from rasa_chatbot_userdatabase WHERE username='{}'".format(slot_value))
        row = cursor.fetchone()
        con.close()
        if row == None:
            dispatcher.utter_message(
                text="Username is not valid. Kindly contact admin")
    ...
    def validate_password(self,
        ...
        con = sqlite3.connect("../Django/db.sqlite3")
        cursor = con.execute(
            "SELECT password from rasa_chatbot_userdatabase WHERE username='{}'".format(slot_value))
        row = cursor.fetchone()
        con.close()
        # if the name is not valid stock tick
        if row == None:
            dispatcher.utter_message(
                text="Password is not valid. Kindly contact admin")
...
class Option3_add(FormValidationAction):
    ...
    def validate_newstock3(self,
        ...
        if authentication == False:
            ...
            con = sqlite3.connect("../Django/db.sqlite3")
            cursor = con.execute(
                "SELECT company_official from rasa_chatbot_bursalist WHERE company='{}'".format(slot_value.lower()))
            row = cursor.fetchone()
            if row == None:
                dispatcher.utter_message(
                    text="Kindly only key in the tick name/Stock Name. \nEg. Seven Eleven/7-ELEVEN MALAYSIA HOLDINGS BERHAD")
                ...
                cursor = con.execute(
                    "SELECT company_official from rasa_chatbot_option3_stock_monitoring WHERE company_official='{}'".format(row[0]))
                ...
                if len(check3) > 10:
                    buttons = [
                        {"title": "Back to Menu", "payload": "/option3"}]
                    button_type = "vertical"
                    dispatcher.utter_message(
                        text="Maximum Stock for Monitoring is 10", buttons=buttons, button_type=button_type)
                    ...
                    button_type = "vertical"
                    dispatcher.utter_message(
                        text="Stock is existed in the Monitoring list", buttons=buttons, button_type=button_type)
                    return {"newstock3": "Invalid"}
                else:
    ...
    def validate_deletestock3(self,
        ...
        if authentication == False:
            ...
            if row == None:
                ...
                cursor = con.execute(
                    "SELECT company_official from rasa_chatbot_option3_stock_monitoring WHERE company_official='{}' AND username='{}'".format(row[0], username))
                check2 = cursor.fetchone()
                con.close()
                if check2 == None:
                    buttons = [
                    ...
                    button_type = "vertical"
                    dispatcher.utter_message(
                        text="Stock is not existed in the Monitoring list", buttons=buttons, button_type=button_type)
...
class Option4_correlation(FormValidationAction):
    ...
    def validate_correlationmonth(self,
        ...
        if not slot_value.isnumeric():
            ...
            dispatcher.utter_message(
                text="Maximum month for correlation analysis is 12")
            return {"correlationmonth": None}
        elif int(slot_value) < 1:
            dispatcher.utter_message(
                text="minimum month for correlation analysis is 1")
            return {"correlationmonth": None}
        else:
        ...
        if authentication == False:
            ...
            if len(stocklist) < 10:
                for stock in stocklist:
                    ...
                    if row == None:
                        dispatcher.utter_message(
                            text="{} is invalid stockname/tick.".format(stock))
                        con.close()
                        return {"stocklist": None}
            else:
                dispatcher.utter_message(
                    text="Currently, it only support maximum 10 stocks to compute optimal portforlio optimization".format(stock))
                con.close()
                return {"stocklist": None}
            con.close()
            dispatcher.utter_message(
                text="Please be patient. Computing Minimum Volatility Portfolio and Optimal Risky Portfolio!".format(stock))
    ...
    def validate_arima_parameters(self,
        ...
        for value in slot_value:
            ...
            if not value.isnumeric():
                dispatcher.utter_message(
                    text="Format is invalid.Kindly follow the following format : eg ARIMA(1,1,0)")
                return {"arima_parameters": None}

        dispatcher.utter_message(
            text="Please be patient. Bot is computing ARIMA forecasting!")
